...
function verReserva(telefone, numreserva){
  ...
  for(let i = 0; i<reservas.length; i++){
    if(reservas[i].telefone == telefone && reservas[i].numreserva == numreserva){
      var btn_pagar = document.getElementById("pagamento_app");
      var modal = document.getElementById("myModal");
      var span = document.getElementsByClassName("close")[0];
      ...
      `;      
      modal.style.display = "block";
      reservas[i].pagou ? btn_pagar.style.display = 'none' : btn_pagar.onclick = function (){
...
function embaralharHorarios (horarios){
